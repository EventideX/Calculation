...
描述: 计算表达式
主要功能包括：中缀转换为后缀、计算后缀表达式
*************************************************************/

#include"head.h"

//运算符栈 
void transExpression(string infix, char postfix[]) //中缀转为后缀 
{
...
//数栈 
string calculateResult(string infix) //计算后缀的值 
{
	...
	while ((postfix[i] != '\0') && (i<1000))
	{
		if ((postfix[i] >= '0') && (postfix[i] <= '9')) //整数入栈 
		{
			...
			else
			{
				...
				case '*':figure[point] = figure[point] * figure[point + 1];
					break;
				case '/':if (figure[point + 1].checkZero()) check = 1; //标识除零 
					figure[point] = figure[point] / figure[point + 1];
				}
			}
		i++;
	}
	if ((check == 0) && (figure[point].checkInt())) return figure[point].transToString(); //判断是否除零 
